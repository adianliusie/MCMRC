...
class Trainer():
    ...
    def model_output(self, batch):
        ...
        loss = output.loss.mean() if self.parallel else output.loss            
    ...
    def save_model(self, name:str='base'):
        if self.parallel:
            model, device = self.model.module, 'cuda'
        else:
            model, device = self.model, next(model.parameters()).device
    ...
    def to(self, device):
        ...
        if device != 'multi_gpu':  
            self.deparallelise_model()
            self.model.to(device)
            self.batcher.to(device)
            self.parallel = False
        else:
            self.parallelise_model()

    def parallelise_model(self):
        if not self.parallel:
            self.model.to('cuda')
            self.model = DataParallel(self.model)
            self.parallel = True
            
    def deparallelise_model(self):
        if self.parallel:
