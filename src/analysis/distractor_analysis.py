...
def anneal_probs(probs:np.ndarray, labels:np.ndarray):
    ...
    avg_prob = np.mean(max_probs)*0.5
...
def mut_info_plot(probs:dict, probs_sh:dict, labels:dict, ax1, ax2, color='blue', calibrate=False):
    ...
    ax1.plot(bin_centres, hist, marker='.', color='green', linewidth=4, linestyle=(0, (3, 1, 1, 1)), markersize=18, label='Count')
    ax2.plot(bin_centres, accuracies,  color='orange', linewidth=4, marker='.', markersize=18, label='Q+{O}+C')
    ax2.plot(bin_centres, accuracies_sh,  color='blue', linewidth=4, marker='.', markersize=18, label='Q+{O}')

#== Count plots for datasets with no labels =====================================================================#

def entropy_count_plot(probs:dict, ax, color='blue'):
    """ get effective number of options distribution """
    probs = convert_dict_np(probs)
    ...
    ax.plot(bin_centres, norm_hist, marker='.', color=color, linewidth=4, markersize=18)

def mut_info_count_plot(probs:dict, probs_sh:dict, ax, color='blue'):
    """ rank points by mutual information, and look at accuracy for each bin """
    probs    = convert_dict_np(probs)
    probs_sh = convert_dict_np(probs_sh)
